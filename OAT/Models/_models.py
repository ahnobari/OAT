...
class ProblemEncoder(nn.Module):
    def __init__(self,
                 ...
                 Cs = [1,2],
                 C_n_layers = [1,1],
                 C_hidden_size = [128,128],
                 C_mapping_size = [64,64],
                 latent_size = 256,
                 **ignore_kwargs):
...
class LatentEncoder(nn.Module):
    def __init__(self, 
        ...
        # VIT-L1
        # self.backbone = EfficientViTLargeBackbone(in_channels=in_channels, width_list=[32, 64, 128, 256, 512], depth_list=[1, 1, 1, 6, 6])
        
        # ResNet 50
        from torchvision.models import resnet50
        self.backbone = resnet50(pretrained=False)
        self.backbone.conv1 = nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.backbone.fc = nn.Linear(in_features=2048, out_features=latent_size)
        
        
        # Linear layers
        self.linear1 = nn.Linear(512, latent_size)
        self.act = nn.SiLU()
        self.linear2 = nn.Linear(latent_size, latent_size)
        
    def forward(self, inputs):
        # x = inputs
        # x = self.backbone(x)['stage_final'].squeeze([2,3])
