from ..core import *
from ..geom._mesh import CuStructuredMesh2D, CuStructuredMesh3D, CuGeneralMesh
import cupy as cp
...
class StructuredStiffnessKernel(StiffnessKernel):
    ...
    def diagonal(self, rho=None):
        if rho is None and not self.has_rho:
            ...
            self.diag =  get_diagonal_node_basis_cuda(
                self.K_single,
                self.elements_flat,
                self.el_ids,
                self.rho,
    ...
    def _matvec(self, rho, vec):
        if not self.has_con:
            self.mat_vec = mat_vec_node_basis_parallel_cuda(
                self.K_single,
                self.elements_flat,
                self.el_ids,
                rho,
    ...
    def _matmat(self, rho, mat, Cp=None):
        if Cp is None:
            if not self.has_con:
                return matmat_node_basis_parallel_cuda(
                    self.K_single,
                    self.elements_flat,
                    self.el_ids,
                    rho,
                ...
                out = matmat_node_basis_parallel_cuda(
                    self.K_single,
                    self.elements_flat,
                    self.el_ids,
                    rho,
            ...
            if not self.has_con:
                return matmat_node_basis_parallel_cuda_(
                    self.K_single,
                    self.elements_flat,
                    self.el_ids,
                    rho,
                ...
                out = matmat_node_basis_parallel_cuda_(
                    self.K_single,
                    self.elements_flat,
                    self.el_ids,
                    rho,
...
class UniformStiffnessKernel(StiffnessKernel):
    ...
    def _matvec(self, rho, vec):
        if not self.has_con:
            self.mat_vec = mat_vec_node_basis_parallel_full_cuda(
                self.Ks,
                self.elements_flat,
                self.el_ids,
                rho,
    ...
    def _matmat(self, rho, mat, Cp=None):
        if Cp is None:
            if not self.has_con:
                return matmat_node_basis_full_parallel_cuda(
                    self.Ks,
                    self.elements_flat,
                    self.el_ids,
                    rho,
                ...
                out = matmat_node_basis_full_parallel_cuda(
                    self.Ks,
                    self.elements_flat,
                    self.el_ids,
                    rho,
            ...
            if not self.has_con:
                return matmat_node_basis_full_parallel_cuda_(
                    self.Ks,
                    self.elements_flat,
                    self.el_ids,
                    rho,
                ...
                out = matmat_node_basis_full_parallel_cuda_(
                    self.Ks,
                    self.elements_flat,
                    self.el_ids,
                    rho,
...
class GeneralStiffnessKernel(StiffnessKernel):
    ...
    def diagonal(self, rho=None):
        if rho is None and not self.has_rho:
            ...
            self.diag =  get_diagonal_node_basis_flat_cuda(
                self.K_flat,
                self.elements_flat,
                self.K_ptr,
                self.elements_ptr,
    ...
    def _matvec(self, rho, vec):
        if not self.has_con:
            self.mat_vec = mat_vec_node_basis_parallel_flat_cuda(
                self.K_flat,
                self.elements_flat,
                self.K_ptr,
                self.elements_ptr,
    ...
    def _matmat(self, rho, mat, Cp=None):
        if Cp is None:
            if not self.has_con:
                return matmat_node_basis_flat_parallel_cuda(
                    self.K_flat,
                    self.elements_flat,
                    self.K_ptr,
                    self.elements_ptr,
                ...
                out = matmat_node_basis_flat_parallel_cuda(
                    self.K_flat,
                    self.elements_flat,
                    self.K_ptr,
                    self.elements_ptr,
            ...
            if not self.has_con:
                return matmat_node_basis_flat_parallel_cuda_(
                    self.K_flat,
                    self.elements_flat,
                    self.K_ptr,
                    self.elements_ptr,
